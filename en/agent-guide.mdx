...
2. Fill in:
   ...
   - **Agent path**
     - macOS/Linux: the output of `which claude-agent-acp`, for example `/usr/local/bin/claude-agent-acp`
     - Windows: the output of `where claude-agent-acp`, for example `C:\Users\<username>\AppData\Roaming\npm\claude-agent-acp.cmd`
...
- macOS: the output of `which claude-agent-acp`, for example `/usr/local/bin/claude-agent-acp`
- Windows: the output of `where claude-agent-acp`, for example `C:\Users\<username>\AppData\Roaming\npm\claude-agent-acp.cmd`
...
Install `@agentclientprotocol/claude-agent-acp`, then configure the `claude-agent-acp` executable path, API key, optional model argument, and a valid `PATH`.
