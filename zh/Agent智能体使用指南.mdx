...
<Tip>
  如果你准备把 OpenClaw 接入 Zditor，可直接查看 [OpenClaw 配置指南](/zh/OpenClaw配置指南)。由于 OpenClaw 提供 `openclaw acp` 桥接命令，因此可以作为 ACP Agent 直接接入。
</Tip>
...
推荐第一次使用的用户，直接按下面 5 步完成 Claude Code 接入：

### 第 1 步：检查是否安装 Node.js

`claude-agent-acp` 依赖 Node.js 运行。先检查本机是否已安装：
...
如果能看到版本号，说明已安装，可以继续下一步。  
如果提示命令不存在，请先到 [Node.js 官网](https://nodejs.org/) 安装 Node.js（建议 `18+` 版本），安装完成后重新打开终端再继续。

### 第 2 步：安装 Claude ACP 适配器
...
如果第二条命令能输出版本号，说明安装成功。

### 第 3 步：找到可执行文件路径

Zditor 需要填写 `claude-agent-acp` 的实际路径：
...
把输出的完整路径复制下来，后面要填到 Zditor。

### 第 4 步：在 Zditor 中填写配置

打开 **设置 → 智能体配置 → 添加**，按下面填写：

- **智能体名称**：`claude-code`
- **智能体路径**：上一步查到的完整路径
- **参数**：可留空
- **环境变量**：
  - macOS/Linux: 先运行 `which node`，如果结果是 `/usr/local/bin/node`，则填写 `ANTHROPIC_API_KEY=your_api_key_here, ANTHROPIC_BASE_URL=https://xaapi.ai, PATH=/usr/local/bin`
  - Windows: 先运行 `where node`，如果结果是 `C:\Program Files\nodejs\node.exe`，则填写 `ANTHROPIC_API_KEY=your_api_key_here, ANTHROPIC_BASE_URL=https://xaapi.ai, PATH=C:\Program Files\nodejs`

你需要先从 [Anthropic Console](https://console.anthropic.com/) 获取 API Key，再填入 `ANTHROPIC_API_KEY`。

<Tip>
  `PATH` 要填写的是 `node` 所在目录，不是 `node` 文件本身。
  例如 `which node` 返回 `/usr/local/bin/node`，那就填 `PATH=/usr/local/bin`。
  如果你使用 `nvm`，通常需要填对应版本目录，例如：
  `PATH=/Users/username/.nvm/versions/node/v18.0.0/bin`
</Tip>
...
1. 如果测试成功，直接保存配置并开始使用
2. 如果测试失败，优先检查：
   - `claude-agent-acp` 路径是否正确
   - `ANTHROPIC_API_KEY` 是否填写正确
   - `PATH` 是否包含 Node.js 所在目录

完成后就可以进入 Agent 模式开始使用。
...
- **格式要求**：
  ...
  - 不能包含制表符或前后空格
- **示例**：
  - macOS: `which claude-agent-acp` 的输出结果，例如 `/usr/local/bin/claude-agent-acp`
  - Windows: `where claude-agent-acp` 的输出结果，例如 `C:\Users\<用户名>\AppData\Roaming\npm\claude-agent-acp.cmd`
...
- **格式**：`键=值` 形式，用英文逗号分隔
- **示例**：
  - `ANTHROPIC_API_KEY=your_key, ANTHROPIC_BASE_URL=https://xaapi.ai, PATH=/usr/local/bin`（假设 `which node` 返回 `/usr/local/bin/node`）
...
2. 在 Zditor 中配置：
   - **智能体名称**：`claude-code`
   - **智能体路径**：
     - macOS/Linux: `which claude-agent-acp` 的输出结果，例如 `/usr/local/bin/claude-agent-acp`
     - Windows: `where claude-agent-acp` 的输出结果，例如 `C:\Users\<用户名>\AppData\Roaming\npm\claude-agent-acp.cmd`
   - **参数**：可留空
   - **环境变量**：
     - macOS/Linux: 如果 `which node` 返回 `/usr/local/bin/node`，则填写 `ANTHROPIC_API_KEY=your_api_key_here, ANTHROPIC_BASE_URL=https://xaapi.ai, PATH=/usr/local/bin`
     - Windows: 如果 `where node` 返回 `C:\Program Files\nodejs\node.exe`，则填写 `ANTHROPIC_API_KEY=your_api_key_here, ANTHROPIC_BASE_URL=https://xaapi.ai, PATH=C:\Program Files\nodejs`
